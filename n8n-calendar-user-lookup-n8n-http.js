...
return (async () => {
  ...
  // Function to get user information from HighLevel API using N8N's HTTP request
  async function getUserInfo(userId) {
    try {
      ...
      console.log(`Response status for ${userId}: ${response.statusCode}`);
      console.log(`User data received for ${userId}:`, JSON.stringify(response.body, null, 2));
      ...
      if (error.statusCode) {
        console.error(`HTTP ${error.statusCode}: ${JSON.stringify(error.response?.body || error.body)}`);
        return {
          userId: userId,
          firstName: 'API Error',
          lastName: `(${error.statusCode})`,
          email: '',
          fullName: `API Error ${error.statusCode} (${userId})`,
  ...
  // Fetch all user information sequentially to avoid rate limiting
  console.log(`Fetching data for ${teamMembers.length} users...`);
  const users = [];
  
  for (let i = 0; i < teamMembers.length; i++) {
    const member = teamMembers[i];
    console.log(`Processing user ${i + 1}/${teamMembers.length}: ${member.userId}`);
  ...
  console.log('All users processed:', JSON.stringify(users, null, 2));
