...
return (async () => {
  ...
  // Function to get user information from HighLevel API using N8N helpers
  async function getUserInfo(userId) {
    try {
      ...
      // Use N8N's httpRequest helper method
      ...
      console.log(`User data received for ${userId}:`, JSON.stringify(response, null, 2));
      ...
      if (error.statusCode || error.response?.statusCode) {
        ...
        const responseBody = error.response?.body || error.body || 'No response body';
        console.error(`HTTP ${statusCode}: ${JSON.stringify(responseBody)}`);
        ...
        return {
          ...
          lastName: `(${statusCode})`,
          email: '',
          fullName: `API Error ${statusCode} (${userId})`,
